...
def search_patient(txt):
    ...
    a = re.search('.*?' + '\s' + 'eligible' + '\s?' + '.*?\.', txt, re.I | re.IGNORECASE)
    strict1 = re.search('.*?' + '\s' + 'patients who had' + '\s?' + '.*?\.', txt, re.I | re.IGNORECASE)
    strict2 = re.search('.*?' + '\s' + 'patients with' + '\s?' + '.*?\.', txt, re.I | re.IGNORECASE)
...
for i in files:
    ...
    tableandchart = table_chart(txt)
    document.add_heading(u'表格相关', 1)
